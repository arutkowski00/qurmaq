...
public class BlockManager : MonoBehaviour
{
    ...
    void Start()
    {
        if (floorCollider == null)
            floorCollider = GameObject.Find("Terrain").collider;
    ...
    void Update()
    {
        ...
        if (GameManager.instance.State == GameState.PLAYING)
        {
            ...
            if (Vector3.Distance(transform.localPosition, originalPosition) > ((BoxCollider)collider).size.x * 1.1f && mode != Mode.FADING)
            {
                mode = Mode.FADING;
    ...
    public void Select()
    {
        ...
        for (int i = 0; i <= 1; i++)
        {
            ...
            var x = ((BoxCollider)collider).size.x - arrow.renderer.bounds.size.x / 2;
            if (i == 1)
                x = -x;
