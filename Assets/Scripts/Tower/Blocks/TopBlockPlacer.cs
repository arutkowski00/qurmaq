...
public class TopBlockPlacer : MonoBehaviour
{
    ...
    void Start()
    {
        for (int x = -1; x <= 1; x++)
        {
            ...
            block.transform.localPosition = new Vector3(x * setupBlockModel.renderer.bounds.size.z, setupBlockModel.renderer.bounds.size.y, 0);
        ...
        targetRow.transform.position = new Vector3(transform.position.x, transform.position.y + renderer.bounds.size.y, transform.position.z);
